...
class BondView(APIView):
    ...
    def get(self, request):
        ...
        bonds = serializers.serialize("json", bonds)

        return Response(data=bonds, status=status.HTTP_200_OK)
    ...
    def post(self, request):
        ...
        if lei_request.status_code == 200:
            ...
            if len(lei_request.json()) > 0:
                ...
                return Response("Invalid bond information", status=status.HTTP_422_UNPROCESSABLE_ENTITY)
            ...
            return Response('HTTP status code: ' + str(lei_request.status_code) + ' - ' + lei_request.json()['message'],
                            status=lei_request.status_code)
